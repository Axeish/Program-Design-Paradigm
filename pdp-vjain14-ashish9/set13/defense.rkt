...
(define Unit<%>
  (interface ()
    ...
    ; EFFECT: Updates the value of base-bounds as recieved from the world
    notified-from-world!
...
(define StatefulWorld% 
  (class* object% (StatefulWorld<%>)
    ...
    ; genreates unit from num-units supplied initially
...
(begin-for-test
  (let ([WORLD1 (mk-world 3 1 3)])
    ...
    (check-equal? (handle-mouse WORLD1 300 300 BTN-DOWN)
                  (handle-mouse WORLD1 300 300 BTN-DOWN)
...
;; In the current design we have implemented a push 
;; and pull design where World pushes its bounds to the unit and pulls score 
;; from the unit. 
...
;; This design would be publisher and subscriber where we would have a 
;; World that is publisher and unit as subscriber. At the same time unit
;; updates the world with the value too and world updates all units with that
;; value.
