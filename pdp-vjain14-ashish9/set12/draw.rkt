...
(require 2htdp/image)
(require rackunit)
(define TIME-ON-TASK 28) ; hours spent
...
(define TRIANGLE-SEL "t")
(define T-LIMIT 80) ; represents constant 0
...
(define TRIANGLE-BOUND 5) ; control region bound of rectangle
...
(define (triangle-sel? sel) (string=? TRIANGLE-SEL sel))
...
(define World% 
  ...
  ;  triangle? : Coordinate Coordinate -> Boolean
  ; Check if the mouse has been clicked in the 
  ; triangle region of the toolbox where mx is the x
  ; coordinate and my is the y coordinate
  ; EXAMPLES:
  (define (triangle? mx my)
    (and (< ZERO mx P-LIMIT)
         (< C-LIMIT my T-LIMIT)))   
                ...
                (cond
                  ...
                  [(rectangle-sel? selection) 
                   (send (create-rectangle-new (list mx my mx my))
                         set-pos-from mx my)]
                  [(triangle-sel? selection) 
                     (create-triangle-new (make-posn mx my) (make-posn mx my))
                            ])))
        ; - IN -
  ...
  (define (render-tools)
    (above (render-text-block POINTER-SEL)
           ...
           (render-text-block CIRCLE-SEL)
            (render-text-block TRIANGLE-SEL)))
...
(define Circle%
  (class* object% (Shape<%>)
    ...
    ; Gets a new circle with the parameters initialized as the ones
    ; passed as the arguments. x = xc, y= yc, r= rad, new-c = nc, 
    ; ev = eve, pos-from = ps-frm
    ...
    (define (btn-down mx my)
      (if
       ...
       (if (on-shape? mx my) (get-new-circle x y r false MOVE (make-posn mx my))
           (get-new-circle x y r false NONE pos-from))))
  ...
  (class* object% (Shape<%>)
    ...
    (define (get-pivot-point mx my)
      (if
       (resizing-from-left-top? mx my)
       (make-posn r b)
       (if (resizing-from-right-top? mx my)
           (make-posn l b)
           (if (resizing-from-left-bottom? mx my)
               (make-posn r t)
               (make-posn l t)))))
...
(define Triangle%
  (class* object% (Shape<%>)
    (init-field cx cy x y new-t ev pos-from)
    
    ;get-first :-> Posn
    ; Returns controlling corner posn
    (define (get-first)
    (first (compute-corners (make-posn cx cy) (make-posn x y))))
    
    ;get-sec :-> Posn
    ; Returns second corner posn
    (define (get-sec)
    (second (compute-corners (make-posn cx cy) (make-posn x y))))
    
    ;get-third :-> Posn
    ; Returns third corner posn
    (define (get-third)
    (third (compute-corners (make-posn cx cy) (make-posn x y))))
    
    ; get-top : -> PosInt
    ; Returns the top - y coordinate of the bounding box    
    (define (get-top)
      (min (posn-y (get-first))
              (posn-y  (get-sec))
                (posn-y (get-third) )))
   ...
   (define (get-left)
    ...
    (define (control-region? mx my)
      (local (;ctrl-each? : Posn Coordinate Coordinate -> Boolean 
              ;Will check for control region of a posn point w.r.t mx and my
              ;Strategy: Data decomposition on point : Posn
              (define (ctrl-each? point mx my)
                (and(<= (left-limit (posn-x point)) 
                        mx 
                        (right-limit (posn-x point)))
                    (<= (left-limit (posn-y point))
                        my
                        (right-limit (posn-y point))))))
        
        ; - IN -
        (or (ctrl-each? (get-first) mx my)
            (ctrl-each? (get-sec) mx my)
            (ctrl-each? (get-third) mx my))))
    ...
    (define (get-moved-coord mc coord)
      (cond
           [(symbol=? coord CX) (+ cx (- mc (posn-x pos-from)))]
           [(symbol=? coord CY) (+ cy (- mc (posn-y pos-from)))]
           [(symbol=? coord X) (+ x (- mc (posn-x pos-from)))]
           [(symbol=? coord Y) (+ y (- mc (posn-y pos-from)))]))
    ...
    ; get-new-triangle : Coordinate Coordinate NonNegReal
    ;                      Boolean ShapeEvent Posn -> Circle<%>
    ; Gets a new triangle with given parameters initialized as
    ; fields of the triangle
    (define (get-new-triangle xc yc x y nc eve ps-frm)
      (new Triangle% [cx xc] [cy yc] 
            [x x] [y y]
           [new-t nc] [ev eve]
           [pos-from ps-frm]))
    ...
    (define (btn-down mx my)
      (if
       (control-region? mx my) (get-new-triangle cx cy mx my 
                                                 false RESIZE pos-from)
       (if 
        (on-shape? mx my) (get-new-triangle cx cy x y false MOVE
                                            (make-posn mx my))
        (get-new-triangle cx cy x y  false NONE pos-from))))
    ...
    (define (btn-drag mx my)
      (cond
          [(resize? ev) (get-new-triangle cx cy mx my 
                                                (if new-t true false)
                                                RESIZE pos-from)]
           [(move? ev) (get-new-triangle (get-moved-coord mx CX)
                                            (get-moved-coord my CY)
                                            (get-moved-coord mx X)
                                            (get-moved-coord my Y)
                                            false MOVE (make-posn mx my))]
                                          
         [(none? ev) (get-new-triangle cx cy x y new-t ev pos-from)]))
   ...
   ; handle-mouse : Coordinate Coordinate MouseEvent -> Circle<%>
    ...
    (define/public (handle-mouse mx my mev)
      (cond
        [(string=? mev BTN-UP) 
         (get-new-triangle cx cy x y false NONE (make-posn ZERO ZERO))]
        [(string=? mev BTN-DOWN) (btn-down mx my)]
        [(string=? mev BTN-DRAG) (btn-drag mx my)]))
   ...
   ; render-shape : Image -> Image
    ...
    (define/public (render-shape scn)
      (if  new-t
           (render-equilateral-triangle (make-posn cx cy)
                                        (make-posn x y) 
                                        OPACITY OP-COL scn)
           (render-equilateral-triangle (make-posn cx cy)
                                        (make-posn x y)
...
;;-----------------------------------------------------------------
; create-triangle : posn Integer -> Shape<%>
; GIVEN: A center point and a radius
; RETURNS: A new Shape<%> object (implementing Shape<%>) with its center at
; the given point and posn as corner.
; Strategy: Data decomposition on pos : Posn
(define (create-triangle cpos pos)
  (new Triangle% 
       [cx (posn-x cpos)]
       [cy (posn-y cpos)]
       [x (posn-x pos)]
...
;;----------------------------------------------------------------------
; create-triangle-new : posn posn -> Shape<%>
; GIVEN: A center point and a corner
; RETURNS: A new Triangle% object (implementing Shape<%>) with its center at
; the given point and pos as given corner.
; Strategy: Data decomposition on pos : Posn
(define (create-triangle-new cpos pos)
  (new Triangle% 
       [cx (posn-x cpos)]
       [cy (posn-y cpos)]
       [x (posn-x pos)]
...
(define TEST-TRI (create-triangle (make-posn 80 80) (make-posn 60 60)))
;; INITIAL-WORLD
...
(define INITIAL-WORLD-TRI 
  (new World% [shapes (list TEST-TRI)] 
       [selection POINTER-SEL]
       [ex-mouse-ev ""]
       [start-posn (make-posn 0 0)]
       [current-posn (make-posn 0 0)]))
...
(define TEST-TRI-SNAP-NEW-RESIZE
  (new Triangle% [cx 80]
       [cy 80]
       [x 155]
       [y 100]
...
(define TEST-TRI-MOVE
  (new Triangle% [cx 80]
       ...
       [new-t false]
       [ev MOVE]
       [pos-from (make-posn 90 70)]))

(define TEST-TRI-MOVED
  (new Triangle% [cx 90]
       [cy 90]
       [x 70]
       [y 70]
...
(define WORLD-TEST1-TRI 
  (new World% [shapes (list TEST-CIRCLE-1)]
       [selection TRIANGLE-SEL]
       [ex-mouse-ev BTN-DOWN ]  
       [start-posn (make-posn 15 70)]
       [current-posn (make-posn 15 70)]))
...
(define WORLD-TEST1-TRI-ADD
  (new World% 
       [shapes 
        (list 
         (create-triangle-new (make-posn 160 160)(make-posn 160 160)) 
         TEST-CIRCLE-1 )]
       [selection TRIANGLE-SEL]
       [ex-mouse-ev BTN-DOWN ]  
       [start-posn (make-posn 160 160)] 
...
(begin-for-test
  (check-false (send TEST-TRI get-new-s)
               "shape already created")
  
  (check-true (send NEW-TRI get-new-s)
              "new shaped")
  
  (check-equal? (send TEST-TRI get-bounds)
                '(60.0
                  60.0
                  107.32050807568876
                  107.32050807568878) 
  ...
  (check-false (shape-equal? TEST-TRI NEW-TRI)
               "shapes are not equal ")
  
  (check-true (shape-equal? (send TEST-TRI handle-mouse 63 64 BTN-DOWN)
                            TEST-TRI-SNAP)
              "triangle is snapped towards first coordinate ")
  
  (check-true (shape-equal? (send TEST-TRI handle-mouse 107 72 BTN-DOWN)
                            TEST-TRI-SNAP-1)
              "triangle is snapped towards first coordinate ")
  (check-true (shape-equal? (send TEST-TRI handle-mouse 75 106 BTN-DOWN)
                            TEST-TRI-SNAP-2)
              "triangle is snapped towards first coordinate ")
  
  (check-true (shape-equal? (send TRI-NONE handle-mouse 220 220 BTN-DOWN)
                            TRI-NONE)
              "out of control ara, world do not change")
  
  (check-true (shape-equal? (send TRI-NONE handle-mouse 20 20 BTN-UP)
                            TRI-NONE)
              "Out of control area , World do not change")
  (check-true (shape-equal? (send TRI-NONE handle-mouse 20 20 BTN-DRAG)
                            TRI-NONE)
              "Out of control area , World do not change")
  
  (check-true (shape-equal? 
               (send TEST-TRI-SNAP handle-mouse 155 100 BTN-DRAG)
               TEST-TRI-SNAP-RESIZE)
              "triangle resized by top right corner")
  
   
  (check-true (shape-equal? 
               (send NEW-TRI handle-mouse 155 100 BTN-DRAG)
               TEST-TRI-SNAP-NEW-RESIZE)
              "New triangle is resized")
  
  
  
  (check-true (shape-equal? (send TEST-TRI handle-mouse 90 70 BTN-DOWN)
                            TEST-TRI-MOVE)
              "triangle selected to be moved or resized")
  (check-true (shape-equal? (send TEST-TRI-MOVE handle-mouse 100 80 BTN-DRAG)
                            TEST-TRI-MOVED)
              "triangle moved"))
...
(begin-for-test
  (check-true (world-equal? (send WORLD-TEST next-world) 
                            WORLD-TEST)
              "world itself")
  (check-true (world-equal? (next-world WORLD-TEST ) 
                            WORLD-TEST)
              "world itself")
  ...
  (check-true (world-equal? (send WORLD-TEST1 
                                  
                                  mouse-handler 15 30 BTN-DOWN)
                            WORLD-TEST1-REC )
  ...
  (check-true (world-equal? (send WORLD-TEST1 mouse-handler 15 70 BTN-DOWN)
                            WORLD-TEST1-TRI )
              "toolbar selection triangle ")
  (check-true (world-equal? 
  ...
  (check-true (world-equal? 
               (send WORLD-TEST1-TRI mouse-handler 160 160 BTN-DOWN)
               WORLD-TEST1-TRI-ADD )
              "addinf rectanngle to the world"))
