...
; <OptrString predicates> : Any -> Boolean
; Returns true if OptrString is the String indicated by the function name.
; STRATEGY: function composition
(define (plus? op) (and (string? op) (string=? op PLUS)))
(define (mult? op) (and (string? op) (string=? op MULT)))
...
; optr? : Any -> ???
; returns true if op is a OptrString
...
(define (strList-for-width s1 s2 s3 s4 width)
  (local ((define str (string-append s1 s2 s3 s4))
...
(define (fit-possible? ind pre se end width)
  (local ((define str (append-to-str ind pre se end))
          (define len (string-length str)))
    ;--IN--
    (<= len width)))
...
(define (get-indent prefix? pre ind)
  (local ((define new-pre (if prefix? 
...
(define (sExpr->strs se0 width)
  (local (; sExpr->strs/a : String String SExpr String -> ListOf<String>
          ...
          (define (sEx-ls->strs ls ind end)
            (local ((define n-end (string-append end CLOSE-STR)))
              (cond
  ...
  (local (
          (define (sExprs->str/a esl a)
            (cond
              [(empty? (rest esl)) 
               (if (cons? (first esl))
                   (string-append a (sExprs->str/a (first esl) "(") ")")
                   (string-append a (first esl) ")"))]
              [else 
               (local ((define new-a 
                         (if (cons? (first esl))
                             (string-append a (sExprs->str/a (first esl) "(")
...
(define (exprNelist->sExpr-infix nel op)
  (local ((define (infix-optr ex b)
            (append (list op (expr->sExpr ex)) b)))
    (append (list (expr->sExpr (first nel)))  
            (foldr infix-optr '() (rest nel)))))
